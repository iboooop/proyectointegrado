...
class ProductoForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'uom_compra', 'uom_venta', 'factor_conversion', 'costo_estandar', 'precio_venta', 'impuesto_iva',
            'stock_minimo', 'stock_maximo', 'punto_reorden', 'perishable', 'control_por_lote', 'control_por_serie',
            'proveedor', 'bodega', 'imagen_url', 'ficha_tecnica_url'
        ]
        widgets = {
            'sku': forms.TextInput(attrs={'class': 'form-control', 'maxlength': 16, 'placeholder': 'SKU único'}),
            'ean_upc': forms.TextInput(attrs={'class': 'form-control', 'maxlength': 13, 'placeholder': 'EAN (13) o UPC (12)'}),
            'nombre': forms.TextInput(attrs={'class': 'form-control', 'maxlength': 255, 'placeholder': 'Nombre del producto'}),
            'descripcion': forms.Textarea(attrs={'class': 'form-control', 'rows': 2, 'maxlength': 255, 'placeholder': 'Descripción (opcional)', 'oninput': 'updateCharCount(this)'}),
            'categoria': forms.Select(attrs={'class': 'form-select'}),
            'marca': forms.TextInput(attrs={'class': 'form-control', 'maxlength': 100, 'placeholder': 'Marca (opcional)'}),
            'modelo': forms.TextInput(attrs={'class': 'form-control', 'maxlength': 100, 'placeholder': 'Modelo (opcional)'}),
            'uom_compra': forms.Select(attrs={'class': 'form-select'}),
            'uom_venta': forms.Select(attrs={'class': 'form-select'}),
            'factor_conversion': forms.NumberInput(attrs={'class': 'form-control', 'min': 1, 'step': '0.001'}),
            'costo_estandar': forms.NumberInput(attrs={'class': 'form-control', 'min': 0, 'step': '0.01'}),
            'precio_venta': forms.NumberInput(attrs={'class': 'form-control', 'min': 0, 'step': '0.01'}),
            'impuesto_iva': forms.NumberInput(attrs={'class': 'form-control', 'min': 0, 'max': 100, 'step': '0.01'}),
    ...
    def clean_sku(self):
        ...
        # Unicidad (Django lo valida, pero por claridad)
        if Producto.objects.filter(sku=sku).exclude(pk=self.instance.pk if self.instance else None).exists():
    ...
    def clean_ean_upc(self):
        ...
        if ean_upc:
            ...
            if Producto.objects.filter(ean_upc=ean_upc).exclude(pk=self.instance.pk if self.instance else None).exists():
